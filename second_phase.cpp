...
void create_bit_vector_3_redundancy(const char* S,vector<int>& icfl_list,int icfl_list_size, suffix_tree_node* root,std::vector<int>& is_custom_suffix){
    vector<int> father_chain = get_chain_from_bit_vector_3(root->father);
    root->bit_vec=in_prefix_merge_bit_vector_5(S,icfl_list,icfl_list_size,father_chain,root->array_of_indexes,root->father->suffix_len,is_custom_suffix);
...
void get_bit_vectors_from_root(const char* S,vector<int>& icfl_list,int icfl_list_size,suffix_tree_node* root,std::vector<int>& is_custom_suffix){
    create_bit_vector_3_redundancy(S,icfl_list,icfl_list_size,root,is_custom_suffix);
    for(int i=0;i<root->sons.size();i++) 
        get_bit_vectors_from_root(S,icfl_list,icfl_list_size,root->sons[i],is_custom_suffix);
    return;
}